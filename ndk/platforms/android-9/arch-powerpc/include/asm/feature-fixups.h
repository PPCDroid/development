...
#define FTR_SECTION_ELSE_NESTED(label)  label##2:   .pushsection __ftr_alt_##label,"a";   .align 2;  label##3:

#define MAKE_FTR_SECTION_ENTRY(msk, val, label, sect)  label##4:   .popsection;   .pushsection sect,"a";   .align 3;  label##5:   FTR_ENTRY_LONG msk;   FTR_ENTRY_LONG val;   FTR_ENTRY_OFFSET label##1b-label##5b;   FTR_ENTRY_OFFSET label##2b-label##5b;   FTR_ENTRY_OFFSET label##3b-label##5b;   FTR_ENTRY_OFFSET label##4b-label##5b;   .ifgt (label##4b- label##3b)-(label##2b- label##1b);   .error "Feature section else case larger than body";   .endif;   .popsection;

#define BEGIN_FTR_SECTION_NESTED(label) START_FTR_SECTION(label)
#define BEGIN_FTR_SECTION START_FTR_SECTION(97)

#define END_FTR_SECTION_NESTED(msk, val, label)   FTR_SECTION_ELSE_NESTED(label)   MAKE_FTR_SECTION_ENTRY(msk, val, label, __ftr_fixup)
...
#define END_MMU_FTR_SECTION_NESTED(msk, val, label)   FTR_SECTION_ELSE_NESTED(label)   MAKE_FTR_SECTION_ENTRY(msk, val, label, __mmu_ftr_fixup)
...
#define END_FW_FTR_SECTION_NESTED(msk, val, label)   FTR_SECTION_ELSE_NESTED(label)   MAKE_FTR_SECTION_ENTRY(msk, val, label, __fw_ftr_fixup)
...
#define ASM_FTR_IF(section_if, section_else, msk, val)   stringify_in_c(BEGIN_FTR_SECTION)   section_if "; "   stringify_in_c(FTR_SECTION_ELSE)   section_else "; "   stringify_in_c(ALT_FTR_SECTION_END((msk), (val)))
...
#define ASM_MMU_FTR_IF(section_if, section_else, msk, val)   stringify_in_c(BEGIN_MMU_FTR_SECTION)   section_if "; "   stringify_in_c(MMU_FTR_SECTION_ELSE)   section_else "; "   stringify_in_c(ALT_MMU_FTR_SECTION_END((msk), (val)))
...
#define MAKE_LWSYNC_SECTION_ENTRY(label, sect)  label##2:   .pushsection sect,"a";   .align 2;  label##3:   FTR_ENTRY_OFFSET label##1b-label##3b;   .popsection;
